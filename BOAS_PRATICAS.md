...
2. **Corte redundância, não clareza.** Prompt enxuto ≠ prompt telegráfico. Encurtar instruções removendo artigos e conectivos ("estilo homem das cavernas") economiza centavos e cria ambiguidade — e instrução ambígua em produção custa horas de debug e comportamento errado, muito mais caro que os tokens poupados. O desperdício real está em: camadas sobrepostas ("este bloco sobrescreve o anterior"), exemplos além do necessário e a mesma regra repetida com palavras diferentes. Consolidar isso corta 30–40% sem perder precisão. Estilo telegráfico só para dados estruturados entre sistemas (JSON, tags, logs) — nunca para instruções. **O mesmo vale para o que o operador digita numa sessão de agente**: a mensagem humana é fração mínima do contexto (o grosso é resultado de ferramenta e histórico) — pedido claro que acerta de primeira economiza muito mais que pedido telegráfico que gera retrabalho. Numa sessão interativa, o que corta custo de verdade é: sessão nova para assunto novo (histórico acumulado é o maior item da conta), apontar o trecho em vez de colar o arquivo, modelo barato para tarefa mecânica, e não deixar a sessão esfriar além do TTL do cache.
